...
#
#  hiplar_eigen.R is based on src/library/base/R/eigen.R
...
eigen <- function(x, symmetric, only.values = FALSE, EISPACK = FALSE)
{
    ...
    if (!EISPACK) {
        if (symmetric) {
            z <- if(!complex.x) 
                .Call("hiplar_rs", x, only.values, PACKAGE = "HiPLARb")
			else 
                .Call("hiplar_rs_cmplx", x, only.values, PACKAGE = "HiPLARb")
			ord <- rev(seq_along(z$values))
        } else {
            z <- if(!complex.x)
                .Call("hiplar_rg", x, only.values, PACKAGE = "HiPLARb")
            else
                .Call("hiplar_rg_cmplx", x, only.values, PACKAGE = "HiPLARb")
