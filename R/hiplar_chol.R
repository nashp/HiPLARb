...
#
#  hiplar_chol.R is based on src/library/base/R/chol.R
...
chol.default <- function(x, pivot = FALSE, LINPACK = pivot, ...)
{
    ...
    if(!LINPACK)
        return(.Call("hiplar_chol", as.matrix(x), pivot, PACKAGE = "HiPLARb"))
...
chol2inv <- function(x, size = NCOL(x), LINPACK = FALSE)
{
    ...
    if(!LINPACK) return(.Call("hiplar_chol2inv", x, size, PACKAGE = "HiPLARb"))
